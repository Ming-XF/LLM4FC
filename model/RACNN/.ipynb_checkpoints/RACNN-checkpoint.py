...
class RACNN(nn.Module):
    ...
    def __init__(self, config: RACNNConfig):
        ...
        self.padLeng = 32
        if config.node_size < self.padLeng:
            config.node_size = self.padLeng
            config.node_feature_size = self.padLeng
    ...
    def forward(self, time_series, labels):
        if time_series.shape[1] < self.padLeng:
            B, C, D = time_series.shape
            zeros_to_add = torch.zeros(B, self.padLeng-C, D, dtype=time_series.dtype, device=time_series.device)
            time_series = torch.cat([time_series, zeros_to_add], dim=1)
