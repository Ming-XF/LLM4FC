...
def init_model_config(args, data_config: DataConfig):
    if args.model == "BNT":
        ...
        model_config = TimeLLMConfig(
            ...
            channel_proto_diversity_weight=args.channel_proto_diversity_weight,
        )
...
def init_config():
    ...
    data_group.add_argument("--drop_last", action="store_true",
                            help="Drop the last incomplete batch (off by default)")
    ...
    model_group.add_argument("--channel_proto_diversity_weight", default=0.0, type=float,
                             help="Barlow-Twins/VICReg covariance regularization on "
                                  "per-channel prototypes (0 = off, try 1e-3 ~ 1e-1)")
    ...
    model_group.add_argument("--lora_num_layers", default=-1, type=int,
                             help="Number of leading LLM layers to inject LoRA into (-1 = all layers)")
