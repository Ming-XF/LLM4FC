...
class DisCAUEEGTUABDataset(BaseDataset):
    """CAUEEG + TUAB 二分类融合（0=正常/对照, 1=患病）。

    domain_label: 0=CAUEEG, 1=TUAB。
    """

    def __init__(self, data_config: DataConfig, k=0, train=True, one_hot=True,
                 episode_seed=None):
        super(DisCAUEEGTUABDataset, self).__init__(data_config, k, train, one_hot=one_hot,
                                                   episode_seed=episode_seed)

    def load_data(self, one_hot=True):
        d1 = dict(np.load("../data/CAUEEG/caueeg_disease.npz", allow_pickle=True))
        d2 = dict(np.load("../data/TUAB/tuab_disease.npz", allow_pickle=True))
