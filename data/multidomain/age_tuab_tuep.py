...
class AgeTUABTUEPDataset(BaseDataset):
    """TUAB + TUEP 年龄回归融合（连续年龄）。
    ...
    def __init__(self, data_config: DataConfig, k=0, train=True, one_hot=True,
                 episode_seed=None):
        super(AgeTUABTUEPDataset, self).__init__(data_config, k, train, one_hot=one_hot,
                                                 episode_seed=episode_seed)

    def load_data(self, one_hot=True):
        d1 = dict(np.load("../data/TUAB/tuab_age.npz", allow_pickle=True))
