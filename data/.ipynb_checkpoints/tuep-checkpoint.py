...
class TUEPDataset(BaseDataset):
    ...
    The dataset is preprocessed by ``tuep_preprocess()`` and stored as a
    single ``.npz`` file.
    """

    def __init__(self, data_config: DataConfig, k=0, train=True, one_hot=True,
                 episode_seed=None):
        super(TUEPDataset, self).__init__(data_config, k, train, one_hot=one_hot,
...
def tuep_preprocess(path="../data/TUEP", hz=200, max_windows_per_subject=60,
                     max_subjects=None):
    ...
    """
    output_path = os.path.join(path, "tuep.npz")
...
if __name__ == '__main__':
    tuep_preprocess("../data/TUEP", hz=200, max_windows_per_subject=(80, None), max_subjects=None)
