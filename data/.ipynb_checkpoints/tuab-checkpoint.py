...
class TUABDataset(BaseDataset):
    ...
    The dataset is preprocessed by ``tuab_preprocess()`` and stored as a
    single ``.npz`` file.
    """

    def __init__(self, data_config: DataConfig, k=0, train=True, one_hot=True,
                 episode_seed=None):
        super(TUABDataset, self).__init__(data_config, k, train, one_hot=one_hot,
...
def tuab_preprocess(path="../data/TUAB", hz=200, max_windows_per_subject=None,
                     max_subjects=None):
    ...
    output_path = os.path.join(path, "tuab.npz")
...
if __name__ == '__main__':
    tuab_preprocess("../data/TUAB", hz=200, max_windows_per_subject=(10, 8), max_subjects=None)
