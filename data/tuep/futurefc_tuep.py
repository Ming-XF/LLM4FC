...
class FutureFCTUEPDataset(BaseDataset):
    """TUEP future FC prediction dataset.
...
def _process_tuep_file_futurefc(args):
    """Process a single TUEP EDF file for future FC prediction.
...
def futurefc_tuep_preprocess(path="../data/TUEP", hz=200,
    ...
    Signal processing identical to ``tuep_preprocess()`` but saves dummy
    ...
    """
    output_path = os.path.join(path, "tuep_futurefc.npz")
    ...
    with Pool(processes=n_workers) as pool:
        for data, labels_arr, subj_arr in tqdm(
                pool.imap_unordered(_process_tuep_file_futurefc, task_args),
                total=len(task_args),
                desc="Processing TUEP futurefc"):
            if data is None:
                skipped += 1
                continue
            ts_list.append(data)
...
if __name__ == '__main__':
    futurefc_tuep_preprocess("../data/TUEP", hz=200)
